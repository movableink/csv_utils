...
impl Validator {
    pub fn new(rules: Vec<ValidationRule>, error_log_path: String) -> Result<Self, ValidationError> {
        ...
        let error_log_file = match File::create(&error_log_path) {
            Ok(mut file) => {
                ...
                if let Err(e) = file.write_all(b"\xEF\xBB\xBF") {
                    return Err(ValidationError { message: format!("Failed to write BOM: {}", e) });
                }
                // Write header
                if let Err(e) = writeln!(file, "Error Message,Row,Column") {
                    return Err(ValidationError { message: format!("Failed to write header: {}", e) });
                }
                Some(file)
            },
            Err(e) => return Err(ValidationError { message: format!("Failed to create error log file: {}", e) })
        };
    ...
    pub fn add_error_to_file(&mut self, error_type: &str, row_number: usize, column: usize, column_name: &str) -> Result<(), ValidationError> {
        if let Some(file) = &mut self.error_log_file {
            let message = match error_type {
                ...
                _ => return Err(ValidationError { message: format!("Unknown error type: {}", error_type) })
            };

            if let Err(e) = writeln!(file, "{},{},{}", message, row_number + 1, column + 1) {
                return Err(ValidationError { message: format!("Failed to write error to log: {}", e) });
            }
        }
        Ok(())
    }
...
pub fn ruby_rules_array_to_rules(rules: RArray) -> Result<Vec<ValidationRule>, Error> {
  ...
  rules.into_iter().map(|rule| {
    ...
    let column_name = rule.aref::<Symbol, Value>(column_name_key)
      .or_else(|_| Err(Error::new(arg_error(), "Missing column_name")))?
      .to_string();
    let validation_type_str = rule.aref::<Symbol, Value>(validation_type_key)
    ...
    match ValidationType::from_string(validation_type_str.as_str()) {
      ...
      validation_type => Ok(ValidationRule { column_name, validation_type })
    }
  }).collect()
}
